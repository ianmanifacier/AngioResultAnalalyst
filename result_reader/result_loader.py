...
class ParameterRanges(dict):
    def __init__(self):
        self.help = "this finds the top and bottom parameter value for a given label"
    def add(self,label,labelvalue):
        if label not in self.keys():
            ...
            v_min = self[label][0]
            v_max = self[label][-1]
            self[label]=[v_min, v_max]
    def max(self, label):
        return self[label][-1]
    def maxRound(self, label):
        return ceil(self[label][-1])
    def min(self, label):
        return self[label][0]
    def minRound(self, label):
        return floor(self[label][0])
    ...
    with open(filename2) as csv_file:
        ...
        for row in csv_reader:
            if line_count ==0:
                ...
                print("     Parameters and their row index:")
                for i in range(0,len(row)):
                    print("     ",i,":", row[i])
                print("\n        ¤ length row 0 : ", len(row))
                parameter_list = row[3:33]
            else:
                if line_count == 1:
                    print("        ¤ length row 1 : ", len(row))
                simKey = int(row[2])
                ...
                myCells[simKey].theta2 = float(row[23])
                myCells[simKey].ruptureForce_N0 = float(row[24])
                myCells[simKey].ruptureForce_N1 = float(row[25])
                myCells[simKey].ruptureForce_N2 = float(row[26])
                myCells[simKey].P_N1 = float(row[27])
                myCells[simKey].P_N20 = float(row[28])
                myCells[simKey].P_N2i = float(row[29])
                myCells[simKey].P_N2m = float(row[30])
                #myCells[simKey].labelRefNb = int(row[31])
                #myCells[simKey].label = row[32]
                myCells[simKey].label_x = row[33]
                myCells[simKey].label_y = row[34]
                if myCells[simKey].simulation_NB != simKey:
                    print("Warning: Error loading file")
            line_count = line_count + 1
        return parameter_ranges, parameter_list, my_subplot_titles, myCells
